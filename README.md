...
| 指标 | Alpha v3 (31因子, 2012-2025) | Alpha v3 样本外 (2024-2026) | Alpha v2 (23因子) |
|------|----------------------------|---------------------------|-------------------|
| 年化收益 | **17.01%** | **35.07%** ⚠️ | 17.2% |
| 最大回撤 | -24.7% | **-15.2%** | -29.8% |
| Sharpe | **0.86** | **1.30** ⚠️ | 0.72 |
| FF5 Alpha | **+10.72% (t=3.34)** | **+26.34% (t=2.08)** ⚠️ | +6.73% (t=2.20) |
| 市场 Beta | 0.42 | 0.43 | 0.82 |
| 下行/上行捕获 | 0.40 / 0.84 | **0.35 / 1.45** | — |

> Alpha v3：31 因子 + trailing 36M 滚动 IC 动态方向。IS: 2012-2023 / OOS: 2024-2026（严格互斥）。
>
> **⚠️ 风险提示：** 回测数字待 IS/OOS 严格分割后重跑填入。样本外高收益（如 35%+）是 AI/科技牛市的风格红利，β_rmw 深度负值说明策略自动加大了低质量成长股权重。滚动 IC 机制捕捉了 Regime，但在风格反转时面临显著回撤。Alpha v4 将研究二次规划优化器施加 β_rmw 约束。
...
| 美股 Alpha v3 | 31 因子 + 9 反转 → FF5α=10.72%(t=3.34)，样本外α=26.34%(t=2.08) ⚠️ | ✅ |
