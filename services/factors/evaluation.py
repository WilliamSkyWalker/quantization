...
class FactorEvaluator:
    ...
    def _get_factor_map(self) -> dict:
        ...
        if self.market == "us":
            ...
            return {
                "EP": value.EP, "BP": value.BP, "DIV_YIELD": dividend.DivYield,
                "ROE_TTM": quality.RoeTTM, "GROSS_MARGIN": quality.GrossMargin,
                "PROFIT_STB": quality.ProfitStability, "MARGIN_TREND": quality.MarginTrend,
                "NET_PROFIT_YOY": growth.NetProfitYoY, "REVENUE_YOY": growth.RevenueYoY,
                "NET_PROFIT_CAGR_3Y": growth.NetProfitCAGR3Y,
                "MOM_1M": momentum.Mom1M, "MOM_3M": momentum.Mom3M,
                "MOM_12M": momentum.Mom12M, "REV_5D": momentum.Rev5D,
                "IND_MOM": momentum.IndMom, "RESIDUAL_MOM": momentum.ResidualMom,
                "CMDTY_MOM": commodity.CmdtyMom,
                "TURN_20D": technical.Turn20D, "VOL_20D": technical.Vol20D,
                "PRICE_DEV_60D": technical.PriceDev60D, "SIZE": technical.Size,
                "VOL_PRICE_DIV": technical.VolPriceDiv,
                "MACRO_CYCLE": macro.MacroCycle, "MACRO_LIQD": macro.MacroLiqd,
                "MACRO_INFL": macro.MacroInfl, "MACRO_EXTR": macro.MacroExtr,
                "POLICY_SENT": sentiment.PolicySent,
                "POLICY_INTENSITY": sentiment.PolicyIntensity,
                "ANALYST_RATING": research.AnalystRating,
                "ANALYST_COVERAGE": research.AnalystCoverage,
            }
    ...
    def _get_price_df(self, start: str, end: str) -> pd.DataFrame:
        ...
        if self.market == "us":
            ...
            df = self.db.query(
                ...
                params={"s": start.replace("-", ""), "e": end.replace("-", "")},
            )
    ...
    def run_all(
        ...
        for i, date_str in enumerate(eval_dates[:-1]):
            ...
            if self.market == "us":
                ...
                d1 = date_str.replace("-", "")
                d2 = next_date.replace("-", "")
                px1 = self.db.query(
                    f"SELECT ts_code, close FROM daily_price WHERE trade_date = '{d1}'"
                )
                px2 = self.db.query(
                    f"SELECT ts_code, close FROM daily_price WHERE trade_date = '{d2}'"
                )
