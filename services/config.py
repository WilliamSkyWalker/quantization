...
# ============================================================
# 券商研报配置
RESEARCH_LOOKBACK_DAYS = int(os.environ.get("RESEARCH_LOOKBACK_DAYS", "120"))
...
COMMODITY_INDUSTRY_MAP = {
    ...
    "J":  {"l1": "煤炭", "l2": "焦炭"},
    "JM": {"l1": "煤炭", "l2": "焦煤"},
    # 能源 → 石油石化 + 公用事业（电力受能源价格影响）
    "SC": {"l1": "石油石化", "l2": "油气开采"},
    # 化工 → 基础化工
    "SA": {"l1": "基础化工", "l2": "纯碱"},
    "MA": {"l1": "基础化工", "l2": "其他化学制品"},
}
# 扩展映射：同一商品可影响多个行业（L1 回退匹配时使用）
# 仅保留强关联映射，避免噪音
COMMODITY_SECONDARY_MAP = {
    "SC": ["基础化工"],         # 原油 → 化工原料（强上下游关系）
    "CU": ["电力设备"],         # 铜 → 电力设备（铜用量最大的行业）
}
...
SIMFIN_DATA_DIR = os.environ.get("SIMFIN_DATA_DIR", str(PROJECT_ROOT / "output" / "simfin_data"))
...
US_CATEGORY_WEIGHTS: dict[str, float] = {
    "value": 1.0, "quality": 1.0, "growth": 1.0,
    "momentum": 1.0, "technical": 1.0,
    "macro": 1.0, "analyst": 1.0, "sentiment": 1.0,
}
