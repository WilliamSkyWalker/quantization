...
class USFinancialData(Base):
    ...
    # --- Income Statement: FMP 全名别名字段 ---
    research_and_development_expenses = Column(Float, comment="研发费用 (FMP 全名)")
    selling_general_and_administrative_expenses = Column(Float, comment="销售管理费用 (FMP 全名)")
    weighted_average_shs_out = Column(Float, comment="加权平均股数 (FMP 全名)")
    ...
    # --- Balance Sheet: FMP 全名别名字段 ---
    cash_and_cash_equivalents = Column(Float, comment="现金及等价物 (FMP 全名)")
    # --- Cash Flow (existing) ---
...
class USKeyMetric(Base):
    ...
    # --- FMP key-metrics: long-name aliases (API 返回全名，与上方缩写字段重复但名称不同) ---
    fiscal_year = Column(String(10), comment="财年")
    capex_to_operating_cash_flow = Column(Float, comment="Capex/经营现金流 (全名)")
    days_of_inventory_outstanding = Column(Float, comment="存货天数 (全名)")
    days_of_payables_outstanding = Column(Float, comment="应付天数 (全名)")
    days_of_sales_outstanding = Column(Float, comment="应收天数 (全名)")
    ev_to_free_cash_flow = Column(Float, comment="EV/FCF (全名)")
    ev_to_operating_cash_flow = Column(Float, comment="EV/OCF (全名)")
    free_cash_flow_yield = Column(Float, comment="自由现金流收益率 (全名)")
    research_and_developement_to_revenue = Column(Float, comment="研发/营收 (全名, FMP 拼写)")
    return_on_assets = Column(Float, comment="ROA (全名)")
    return_on_capital_employed = Column(Float, comment="ROCE (全名)")
    return_on_equity = Column(Float, comment="ROE (全名)")
    return_on_invested_capital = Column(Float, comment="ROIC (全名)")
    sales_general_and_administrative_to_revenue = Column(Float, comment="SGA/营收 (全名)")
    stock_based_compensation_to_revenue = Column(Float, comment="SBC/营收 (全名)")
...
class USFinancialGrowth(Base):
    ...
    # --- FMP 不规则命名别名（API camelCase 转换后与标准 snake_case 不一致） ---
    book_valueper_share_growth = Column(Float, comment="每股账面价值增长率 (FMP 拼写)")
    ebitgrowth = Column(Float, comment="EBIT 增长率 (FMP 拼写)")
    epsdiluted_growth = Column(Float, comment="稀释 EPS 增长率 (FMP 拼写)")
    epsgrowth = Column(Float, comment="EPS 增长率 (FMP 拼写)")
    five_y_dividendper_share_growth_per_share = Column(Float, comment="5年每股股息增长率 (FMP 拼写)")
    rdexpense_growth = Column(Float, comment="研发费用增长率 (FMP 拼写)")
    sgaexpenses_growth = Column(Float, comment="SGA 费用增长率 (FMP 拼写)")
    ten_y_dividendper_share_growth_per_share = Column(Float, comment="10年每股股息增长率 (FMP 拼写)")
    three_y_dividendper_share_growth_per_share = Column(Float, comment="3年每股股息增长率 (FMP 拼写)")
