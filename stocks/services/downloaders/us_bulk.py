...
class BulkDownloader:
    ...
    def download_fmp_company_profiles(self, tickers: list[str] = None) -> int:
        ...
        if not tickers:
            return 0
        # 纯快照表：用 updated_at 判断过期
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USCompanyProfile, "updated_at")
            logger.info(f"增量更新 us_company_profile: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        total = 0
    ...
    def download_fmp_financial_scores(self, tickers: list[str] = None) -> int:
        ...
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USFinancialScore, "updated_at")
            logger.info(f"增量更新 us_financial_score: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
    ...
    def download_fmp_shares_float(self, tickers: list[str] = None) -> int:
        ...
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USSharesFloat, "date")
            logger.info(f"增量更新 us_shares_float: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
    ...
    def download_fmp_employee_count(self, tickers: list[str] = None) -> int:
        ...
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USEmployeeCount, "period_of_report")
            logger.info(f"增量更新 us_employee_count: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
        ...
        def _fetch(ticker):
            if latest_map is not None and self._ticker_needs_update(ticker, latest_map, stale_days=90) is None:
                return 0
    ...
    def download_fmp_price_targets(self, tickers: list[str] = None) -> int:
        ...
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USPriceTarget, "updated_at")
            logger.info(f"增量更新 us_price_target: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
    ...
    def download_fmp_esg_ratings(self, tickers: list[str] = None) -> int:
        ...
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USESGRating, "updated_at")
            logger.info(f"增量更新 us_esg_rating: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
        ...
        def _fetch(ticker):
            if latest_map is not None and self._ticker_needs_update(ticker, latest_map, stale_days=365) is None:
                return 0
    ...
    def download_fmp_dcf_valuations(self, tickers: list[str] = None) -> int:
        ...
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USDCFValuation, "date")
            logger.info(f"增量更新 us_dcf_valuation: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
    ...
    def download_fmp_stock_peers(self, tickers: list[str] = None) -> int:
        ...
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USStockPeer, "updated_at")
            logger.info(f"增量更新 us_stock_peer: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
    ...
    # --- 31. press_release (per-ticker, 同 earnings 骨架) ---
    def download_fmp_press_releases(self, tickers: list[str] = None) -> int:
        if tickers is None:
            tickers = self._get_tickers(stocks_only=True)
        if not tickers:
            return 0
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USPressRelease, "updated_at")
            logger.info(f"增量更新 us_press_release: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
            tickers = self._skip_done_tickers("us_press_release", tickers)
            if not tickers:
                return 0
        total = 0
        ...
        def _fetch(ticker):
            if latest_map is not None and self._ticker_needs_update(ticker, latest_map, stale_days=7) is None:
                return 0
            data = self._fmp_get_stable("news/press-releases", params={"symbols": ticker, "limit": 500})
            if not data:
                return 0
            df = _fmp_df_to_snake(pd.DataFrame(data))
            df["ticker"] = ticker
            ...
            if df.empty:
                return 0
            self._um.upsert_df(USPressRelease, df, ["ticker", "url"])
            return len(df)

        with ThreadPoolExecutor(max_workers=self._workers) as pool:
            futures = {pool.submit(self._mark_done, "us_press_release", _fetch, t): t for t in tickers}
            for f in tqdm(as_completed(futures), total=len(futures), desc="FMP Press Releases"):
                try:
                    total += f.result()
                except Exception as e:
                    logger.warning(f"Press release 失败 {futures[f]}: {e}")
        logger.info(f"FMP press_release 总计: {total} 条")
        return total

    # --- 32. sec_filing (per-ticker, 同 earnings 骨架; 全跨度一次翻页) ---
    def download_fmp_sec_filings(self, tickers: list[str] = None) -> int:
        if tickers is None:
            tickers = self._get_tickers(stocks_only=True)
        if not tickers:
            return 0
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USSecFiling, "updated_at")
            logger.info(f"增量更新 us_sec_filing: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
            tickers = self._skip_done_tickers("us_sec_filing", tickers)
            if not tickers:
                return 0
        total = 0
        ...
        def _fetch(ticker):
            ...
            page = 0
            while True:
                data = self._fmp_get_stable("sec-filings-search/symbol", params={
                    "symbol": ticker, "from": "2000-01-01", "to": "2099-12-31",
                    "page": page, "limit": 500,
                })
                if not data:
                    break
            ...
            if not all_data:
                return 0
            df = _fmp_df_to_snake(pd.DataFrame(all_data))
            df["ticker"] = ticker
            # API 返回 form_type/final_link/link → model 是 type/url/title
            df = df.rename(columns={"form_type": "type", "final_link": "url", "link": "title"})
            df = df[df["filing_date"].notna()].copy()
            if df.empty:
                return 0
            self._um.upsert_df(USSecFiling, df, ["ticker", "filing_date", "url"])
            return len(df)

        with ThreadPoolExecutor(max_workers=self._workers) as pool:
            futures = {pool.submit(self._mark_done, "us_sec_filing", _fetch, t): t for t in tickers}
            for f in tqdm(as_completed(futures), total=len(futures), desc="FMP SEC Filings"):
                try:
                    total += f.result()
                except Exception as e:
                    logger.warning(f"SEC filing 失败 {futures[f]}: {e}")
        logger.info(f"FMP sec_filing 总计: {total} 条")
        return total

    # --- 33. revenue_segment (per-ticker, 2 个端点合并后一次 upsert) ---
    def download_fmp_revenue_segments(self, tickers: list[str] = None) -> int:
        if tickers is None:
            tickers = self._get_tickers(stocks_only=True)
        if not tickers:
            return 0
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USRevenueSegment, "updated_at")
            logger.info(f"增量更新 us_revenue_segment: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
            tickers = self._skip_done_tickers("us_revenue_segment", tickers)
            if not tickers:
                return 0
        total = 0

        def _flatten(data, ticker, seg_type):
            """把 FMP 嵌套返回扁平化成 rows list."""
            rows = []
            for period in (data if isinstance(data, list) else []):
                date = period.get("date")
                segments = period.get("data") or period.get("segments") or {}
                if not isinstance(segments, dict):
                    continue
                for seg_name, revenue in segments.items():
                    rows.append({"ticker": ticker, "date": date, "segment_type": seg_type,
                                 "segment_name": str(seg_name)[:300], "revenue": revenue})
            return rows
        ...
        def _fetch(ticker):
            if latest_map is not None and self._ticker_needs_update(ticker, latest_map, stale_days=30) is None:
                return 0
            rows = []
            d1 = self._fmp_get_stable("revenue-product-segmentation", params={"symbol": ticker})
            rows.extend(_flatten(d1, ticker, "product"))
            d2 = self._fmp_get_stable("revenue-geographic-segmentation", params={"symbol": ticker})
            rows.extend(_flatten(d2, ticker, "geographic"))
            if not rows:
                return 0
            ...
            if df.empty:
                return 0
            self._um.upsert_df(USRevenueSegment, df, ["ticker", "date", "segment_type", "segment_name"])
            return len(df)

        with ThreadPoolExecutor(max_workers=self._workers) as pool:
            futures = {pool.submit(self._mark_done, "us_revenue_segment", _fetch, t): t for t in tickers}
            for f in tqdm(as_completed(futures), total=len(futures), desc="FMP Revenue Segments"):
                try:
                    total += f.result()
                except Exception as e:
                    logger.warning(f"Revenue segment 失败 {futures[f]}: {e}")
        logger.info(f"FMP revenue_segment 总计: {total} 条")
        return total

    # --- 34. institutional_holder (per-ticker, Quiver sec13fchanges 一次全量 + 自聚合) ---
    def download_quiver_institutional_holders(self, tickers: list[str] = None) -> int:
        """Quiver /live/sec13fchanges per-ticker: 一次返回所有季度的 per-fund 明细，
        自聚合为 USInstitutionalHolder 汇总（investors_holding / total_shares / new_positions 等）。
        """
        if tickers is None:
            tickers = self._get_tickers(stocks_only=True)
        if not tickers:
            logger.warning("download_quiver_institutional_holders: 无 ticker，跳过")
            return 0
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USInstitutionalHolder, "date")
            logger.info(f"增量更新 us_institutional_holder: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
            tickers = self._skip_done_tickers("us_institutional_holder", tickers)
            if not tickers:
                logger.info("download_quiver_institutional_holders: 全部 ticker 已完成，跳过")
                return 0
        total = 0
        ...
        def _fetch(ticker):
            if latest_map is not None and self._ticker_needs_update(ticker, latest_map, stale_days=30) is None:
                return 0
            data = self._quiver_get_json("live/sec13fchanges", params={"ticker": ticker})
            if not data:
                return 0
            ...
            df["Held"] = pd.to_numeric(df.get("Held", 0), errors="coerce").fillna(0)
            df["Change"] = pd.to_numeric(df.get("Change", 0), errors="coerce").fillna(0)
            df["Change_Share"] = pd.to_numeric(df.get("Change_Share", 0), errors="coerce").fillna(0)
            df["Close"] = pd.to_numeric(df.get("Close", 0), errors="coerce").fillna(0)
            df = df[df["ReportPeriod"].notna()]
            if df.empty:
                return 0

            rows = []
            for period, grp in df.groupby("ReportPeriod"):
                n_funds = len(grp)
                total_shares = grp["Held"].sum()
                total_value = (grp["Held"] * grp["Close"]).sum()
                new_pos = int((grp["Change_Share"] > 0).sum() & (grp["Held"] == grp["Change_Share"]).sum())  # 新建仓近似
                increased = int((grp["Change_Share"] > 0).sum())
                reduced = int((grp["Change_Share"] < 0).sum())
                closed = int((grp["Held"] == 0).sum())
                # put/call 暂无
                rows.append({
                    "ticker": ticker,
                    "date": period.date(),
            ...
            result_df["investors_holding_change"] = result_df["investors_holding"].diff()
            result_df["number_of_13f_shares_change"] = result_df["number_of_13f_shares"].diff()
            result_df["total_invested_change"] = result_df["total_invested"].diff()
            self._um.upsert_df(USInstitutionalHolder, result_df, ["ticker", "date"])
            return len(result_df)

        with ThreadPoolExecutor(max_workers=self._workers) as pool:
            futures = {pool.submit(self._mark_done, "us_institutional_holder", _fetch, t): t for t in tickers}
            for f in tqdm(as_completed(futures), total=len(futures), desc="Quiver 13F Holdings"):
                try:
                    total += f.result()
                except Exception as e:
                    logger.warning(f"13F holdings 失败 {futures[f]}: {e}")
        logger.info(f"Quiver institutional_holder 总计: {total} 条")
        return total
    ...
    # --- Quiver: dark_pool_volume (同 lobbying 骨架) ---
    def download_quiver_dark_pool(self, tickers: list[str] = None) -> int:
        """Quiver /historical/offexchange/{ticker}: 日频 dark pool (short interest 日频代理)"""
        if tickers is None:
            tickers = self._get_tickers(stocks_only=True)
        if not tickers:
            logger.warning("download_quiver_dark_pool: 无 ticker 列表，跳过")
            return 0
        latest_map = None
        if self._incremental:
            latest_map = self._get_ticker_latest(USDarkPoolVolume, "date")
            logger.info(f"增量更新 us_dark_pool_volume: {len(tickers)} tickers, DB 已有 {len(latest_map)} tickers 有数据")
        else:
            tickers = self._skip_done_tickers("us_dark_pool_volume", tickers)
            if not tickers:
                logger.info("download_quiver_dark_pool: 全部 ticker 已完成，跳过")
                return 0
        total = 0
        ...
        def _fetch(ticker):
            if latest_map is not None and self._ticker_needs_update(ticker, latest_map, stale_days=7) is None:
                return 0
            data = self._quiver_get_json(f"historical/offexchange/{ticker}")
            if not data:
                return 0
            df = pd.DataFrame(data)
            df = df.rename(columns={
                "Date": "date", "OTC_Short": "otc_short",
                "OTC_Total": "otc_total", "DPI": "dpi", "Ticker": "ticker",
            })
            df["ticker"] = ticker
            ...
            if df.empty:
                return 0
            df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.date
            df["otc_short"] = pd.to_numeric(df["otc_short"], errors="coerce")
            df["otc_total"] = pd.to_numeric(df["otc_total"], errors="coerce")
            df["dpi"] = pd.to_numeric(df["dpi"], errors="coerce")
            df = df[df["date"].notna()]
            if df.empty:
                return 0
            self._um.upsert_df(USDarkPoolVolume, df, ["ticker", "date"])
            return len(df)

        with ThreadPoolExecutor(max_workers=self._workers) as pool:
            futures = {pool.submit(self._mark_done, "us_dark_pool_volume", _fetch, t): t for t in tickers}
            for f in tqdm(as_completed(futures), total=len(futures), desc="Quiver Dark Pool"):
                try:
                    total += f.result()
                except Exception as e:
                    logger.warning(f"Dark pool 失败 {futures[f]}: {e}")
        logger.info(f"Quiver dark_pool_volume 总计: {total} 条")
        return total
    ...
    def download_quiver_all(self) -> dict:
        ...
        logger.info("=== Quiver: dark_pool ===")
        results["dark_pool"] = self.download_quiver_dark_pool()
        logger.info("=== Quiver: 13f_holdings ===")
        results["13f_holdings"] = self.download_quiver_institutional_holders()
        return results
