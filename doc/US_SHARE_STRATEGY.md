...
| **Step 5** | Alpha v3: 31 因子 + 9 反转 + 数据修复 | ✅ **alpha=+10.72%（t=3.34），Sharpe 0.86，DD -24.7%** |
...
|------|----------------------------|-------------------|------|
| 年化收益 | **17.01%** | 17.2% | 持平 |
| 最大回撤 | -24.7% | -29.8% | **改善 5.1%** |
| Sharpe | **0.86** | 0.72 | **+0.14** |
| FF5 Alpha | **+10.72% (t=3.34)** | +6.73% (t=2.20) | **Alpha +4%, t 值大幅提升** |
| 超额年化 | +4.32% | +7.53% | -3.21% |
| β_mkt | 0.42 | 0.82 | **大幅降低** |
| β_hml | -0.11 | — | 价值因子反转确认 |
| β_rmw | **-0.35** | — | ⚠️ 做空高盈利，与 quality 设计矛盾 |
| R² | **0.37** | — | FF5 解释力更高 |
| 下行/上行捕获 | **0.40 / 0.84** | — | 跌少涨多 |

**关键发现：**
- **FF5 Alpha 提升（6.73%→10.72%）**，t=3.34 在 1% 置信度下显著
...
### Step 5 样本外验证（2024-01 ~ 2026-03）

| 指标 | v3 样本外（修复后） | v3 样本外（修复前） | v2 样本外 |
|------|-------------------|-------------------|----------|
| 年化收益 | **35.07%** ⚠️ | 36.24% | 12.16% |
| FF5 Alpha | **+26.34% (t=2.08)** | +25.32% (t=1.59) | -1.61% (t=-0.23) |
| Sharpe | **1.30** | 1.15 | 0.51 |
| 最大回撤 | **-15.2%** | -19.6% | -20.6% |
| β_mkt | 0.43 | 0.44 | 0.65 |
| β_rmw | **-0.89** | -1.02 | — |
| 下行捕获 | **0.35** | -0.01 | 99.4% |
| 上行捕获 | **1.45** | 1.45 | — |

**样本外结论：**
- v2 的核心问题已解决——alpha 从 -1.61% 恢复到 +26.34%，t=2.08 刚过 5% 显著性
- 修复 Insider 前视偏差后 t 值反而提升（1.59→2.08），说明前视偏差是噪音而非帮助
- 下行捕获从 99.4%→0.35，市场下跌时策略大幅少跌
- 最大回撤从 -19.6% 改善到 -15.2%
