...
| **Step 5** | Alpha v3: 31 因子 + 滚动 IC 动态方向 | ✅ IS α=6.68%(t=2.02)，OOS α=15.06%(t=1.86) |
...
**⚠️ 时间线定义（严格互斥）：**
- **样本内 (IS)**：2012-01-01 ~ 2023-12-31（策略开发、因子筛选的全部决策截止于此）
- **样本外 (OOS)**：2024-01-01 ~ 2026-03-31（系统逻辑在 2023 年底锁死，此后纯粹验证）

| 指标 | IS (2012-2023) | OOS (2024-2026) | Alpha v2 IS (2015-2025) |
|------|---------------|-----------------|------------------------|
| 年化收益 | 14.39% | **20.95%** | 17.2% |
| 最大回撤 | -25.6% | **-18.8%** | -29.8% |
| Sharpe | 0.74 | **0.87** | 0.72 |
| FF5 Alpha | **+6.68% (t=2.02)** | +15.06% (t=1.86) | +6.73% (t=2.20) |
| 超额年化 | +0.20% | **+12.08%** | +7.53% |
| β_mkt | 0.48 | 0.46 | 0.82 |
| β_rmw | -0.31 | -0.42 | — |
| 下行/上行捕获 | 0.46 / 0.77 | 0.44 / 0.97 | — |

**IS 结论：**
- FF5 Alpha=6.68%(t=2.02) 刚过 5% 显著性，与 v2 的 6.73%(t=2.20) 基本持平
- 超额年化仅 +0.20%（几乎没跑赢 Russell 1000），alpha 来自 FF5 无法解释的残差
- β_mkt=0.48（比 v2 的 0.82 大幅降低），策略从"接近主动多头"变为真正中性
- β_rmw=-0.31 仍然偏负，但在可接受范围

**OOS 结论：**
- Alpha=15.06%(t=1.86) 未达 5% 显著性，但方向正确且远好于 v2 的 -1.61%
- 年化 20.95% 超额 +12.08%，主要受 AI/科技牛市风格红利驱动
- β_rmw=-0.42 说明滚动 IC 自动加大了低质量成长股权重（参见下方解释）
- 下行捕获 0.44 稳定，跌的时候只跌一半
...
**⚠️ 风险提示：**
1. **IS/OOS 必须严格互斥**：之前的 2012-2025 IS + 2024-2026 OOS 存在数据泄露，已修正为 2012-2023 / 2024-2026
...
未来 Alpha v4 将研究引入二次规划优化器 (Quadratic Optimizer)，在选股权重分配阶段施加严格的 β_mkt 和 β_rmw 组合级约束，从而在牺牲部分弹性收益的前提下，追求更加纯粹的市场中性 Alpha。短期替代方案：提升 quality 大类权重（2.0~3.0），强行对冲动量和成长带来的低质量偏移。
