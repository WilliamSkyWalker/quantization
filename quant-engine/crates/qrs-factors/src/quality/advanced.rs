...
fn fin_field(cache: &DataCache, tid: qrs_core::types::TickerId, date: Date, field: &str) -> Option<f64> {
...
fn fin_n_quarters(cache: &DataCache, tid: qrs_core::types::TickerId, date: Date, field: &str, n: usize) -> Vec<f64> {
...
impl Factor for OhlsonO {
    ...
    fn compute(&self, date: Date, cache: &DataCache) -> FactorResult {
        ...
        for (&tid, records) in &cache.financials {
            ...
            let f = |r: &qrs_data::cache::FinancialRecord, s: &str| r.fields.get(s).copied().filter(|v| v.is_finite());
...
impl Factor for MarginTrend {
    ...
    fn compute(&self, date: Date, cache: &DataCache) -> FactorResult {
        ...
        for (&tid, records) in &cache.financials {
            let recent: Vec<_> = records.iter().filter(|r| r.filing_date <= date).take(2).collect();
            if recent.len() < 2 { continue; }
            let gm = |r: &qrs_data::cache::FinancialRecord| -> Option<f64> {
