...
impl Factor for IndustryMom {
    ...
    fn compute(&self, date: Date, cache: &DataCache) -> FactorResult {
        ...
        let mut returns: rustc_hash::FxHashMap<quant_core::types::TickerId, f64> = Default::default();
        let tickers_on_date: Vec<_> = cache.daily_prices.iter_date(date)
            .map(|(tid, _)| tid)
            .collect();
        ...
        let mut sector_returns: rustc_hash::FxHashMap<quant_core::types::SectorId, Vec<f64>> = Default::default();
        ...
        let mut sector_medians: rustc_hash::FxHashMap<quant_core::types::SectorId, f64> = Default::default();
