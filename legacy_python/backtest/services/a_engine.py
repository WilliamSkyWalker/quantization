...
from stocks.models import ADailyPrice, AIndexDaily
...
class BacktestEngine:
    ...
    def _get_trade_dates(self, start_date: str, end_date: str) -> pd.DatetimeIndex:
        ...
        if not dates:
            logger.debug("_get_trade_dates: 无交易日数据")
    ...
    def _load_prices(
        ...
        df = pd.DataFrame(_rows) if _rows else pd.DataFrame()
    ...
    def _get_benchmark_nav(self, start_date: str, end_date: str) -> pd.Series:
        ...
        for code in [f"{benchmark_code}.SH", f"{benchmark_code}.SZ", benchmark_code]:
            _rows = list(
                AIndexDaily.objects.filter(
                    ts_code=code, trade_date__gte=s_d, trade_date__lte=e_d,
                ).order_by("trade_date").values("trade_date", "pct_chg")
            )
            df = pd.DataFrame(_rows) if _rows else pd.DataFrame()
            if not df.empty:
        ...
        # 回退：全市场等权平均（来自 ADailyPrice）
        logger.warning("未找到沪深300指数，回退到全市场等权平均（建议跑 bulk_import --source tushare --target index）")
        _agg = list(
            ADailyPrice.objects.filter(trade_date__gte=s_d, trade_date__lte=e_d)
            .values("trade_date").annotate(avg_ret=Avg("pct_chg")).order_by("trade_date")
        )
        df = pd.DataFrame(_agg) if _agg else pd.DataFrame()
    ...
    def _get_industry_benchmark_navs(
        ...
        for industry in ALLOWED_INDUSTRIES:
            ...
            _s_d = pd.to_datetime(start_date).date()
            _e_d = pd.to_datetime(end_date).date()
            _rows = list(
                AIndexDaily.objects.filter(
                    ts_code=index_code, trade_date__gte=_s_d, trade_date__lte=_e_d,
                ).order_by("trade_date").values("trade_date", "pct_chg")
            )
            df = pd.DataFrame(_rows) if _rows else pd.DataFrame()
            if df.empty:
