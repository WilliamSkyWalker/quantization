...
# DatabaseManager 已废弃，改用 Django ORM
from stocks.models import ADailyPrice, AFinancialIncome, AStockBasic
from stocks.services.a_cleaner import get_clean_universe, preload_clean_universes  # TODO P2: 迁 cleaner
from stocks.services.factors.a_value import EPFactor, BPFactor
from stocks.services.factors.a_dividend import DividendYieldFactor
from stocks.services.factors.a_momentum import MOM1MFactor, MOM3MFactor, MOM12MFactor, ShortReversalFactor, ResidualMomentumFactor
from stocks.services.factors.a_quality import ROEFactor, GrossMarginFactor, ProfitStabilityFactor, MarginTrendFactor
from stocks.services.factors.a_growth import NetProfitYOYFactor, RevenueYOYFactor, NetProfitCAGR3YFactor
from stocks.services.factors.a_technical import (
    Turnover20DFactor,
    VolatilityFactor,
...
from stocks.services.factors.a_commodity import CommodityMomentumFactor
from stocks.services.factors.a_macro import (
    MacroCycleFactor,
    MacroLiquidityFactor,
    MacroInflationFactor,
    MacroExternalFactor,
)
from stocks.services.factors.a_sentiment import (
    SentimentPolicyFactor,
    SentimentIntensityFactor,
)
from stocks.services.factors.a_research import (
    AnalystRatingFactor,
    AnalystCoverageFactor,
)
from stocks.services.factors.a_base import FactorBase
from stocks.services.factors.a_processor import process_factor, clear_neutralize_cache
from backtest.services.a_regime import RegimeDetector
...
class MultiFactorStrategy:
    ...
    def _apply_financial_staleness_decay(
        ...
        if bulk_fin is not None and not bulk_fin.empty:
            ...
            _rows = list(
                AFinancialIncome.objects.filter(
                    ts_code__in=codes, ann_date__lte=d,
                ).values("ts_code").annotate(latest_end_date=_Max("end_date"))
            )
            df_latest = pd.DataFrame(_rows) if _rows else pd.DataFrame()
    ...
    def get_rebalance_dates(self, start_date: str, end_date: str) -> list[str]:
        ...
        _dates = list(
            ADailyPrice.objects.filter(trade_date__gte=s_d, trade_date__lte=e_d)
            .values_list("trade_date", flat=True).distinct().order_by("trade_date")
        )
        df = pd.DataFrame({"trade_date": _dates}) if _dates else pd.DataFrame()
    ...
    def _generate_signals_adaptive(
        ...
        # 为了计算最短间隔，获取全部交易日列表
        if sorted_dates:
            _s_d = pd.to_datetime(sorted_dates[0]).date()
            _e_d = pd.to_datetime(sorted_dates[-1]).date()
            _all_tds = list(
                ADailyPrice.objects.filter(trade_date__gte=_s_d, trade_date__lte=_e_d)
                .values_list("trade_date", flat=True).distinct().order_by("trade_date")
            )
            trade_date_list = [d.strftime("%Y-%m-%d") for d in _all_tds]
        else:
            trade_date_list = []
