...
from stocks.models import ADailyPrice, AIndustryClass, AStockBasic
...
def get_clean_universe(
    ...
    # 1. 获取基本信息，过滤退市、ST、新股
    # ----------------------------------------------------------
    df_basic = pd.DataFrame(list(
        AStockBasic.objects.values(
    ...
    # 2. 获取当日行情，过滤停牌
    # ----------------------------------------------------------
    _td = pd.to_datetime(target_date_str).date()
    df_daily = pd.DataFrame(list(
        ADailyPrice.objects.filter(
            ts_code__in=stock_codes, trade_date=_td,
        ).values("ts_code", "vol", "amount", "pct_chg", "is_limit_up", "is_limit_down")
    ))
    ...
    if min_turnover and min_turnover > 0:
        lookback_start = (target_dt - timedelta(days=lookback_days * 2))
        df_amount = pd.DataFrame(list(
            ADailyPrice.objects.filter(
                ts_code__in=stock_codes,
                trade_date__gte=lookback_start, trade_date__lte=_td,
            ).values("ts_code", "trade_date", "amount")
        ))
    ...
    if MIN_MARKET_CAP and MIN_MARKET_CAP > 0:
        codes_mv = df_daily["ts_code"].tolist()
        price_rows = list(
            ADailyPrice.objects.filter(
                ts_code__in=codes_mv, trade_date=_td,
            ).values("ts_code", "close")
        )
        share_rows = list(
            AStockBasic.objects.filter(ts_code__in=codes_mv).values("ts_code", "total_share")
        )
        df_price_mv = pd.DataFrame(price_rows) if price_rows else pd.DataFrame()
        df_share_mv = pd.DataFrame(share_rows) if share_rows else pd.DataFrame()
        if not df_price_mv.empty and not df_share_mv.empty:
            df_mv = df_price_mv.merge(df_share_mv, on="ts_code", how="inner")
        else:
            df_mv = pd.DataFrame()
    ...
    # 合并行业（申万 L1 + L2）
    try:
        l1_rows = list(AIndustryClass.objects.filter(
            src="SW2021", level="L1", out_date__isnull=True,
        ).values("ts_code", "index_name"))
        l2_rows = list(AIndustryClass.objects.filter(
            src="SW2021", level="L2", out_date__isnull=True,
        ).values("ts_code", "index_name"))
        df_l1 = pd.DataFrame(l1_rows).rename(columns={"index_name": "industry_name"}) if l1_rows else pd.DataFrame(columns=["ts_code", "industry_name"])
        df_l2 = pd.DataFrame(l2_rows).rename(columns={"index_name": "l2_industry_name"}) if l2_rows else pd.DataFrame(columns=["ts_code", "l2_industry_name"])
        if not df_l1.empty:
            df_result = df_result.merge(df_l1, on="ts_code", how="left")
        else:
            df_result["industry_name"] = None
        if not df_l2.empty:
            df_result = df_result.merge(df_l2, on="ts_code", how="left")
...
def preload_clean_universes(
    ...
    df_basic = pd.DataFrame(list(
        AStockBasic.objects.values(
            "ts_code", "name", "market", "list_date", "delist_date", "is_st", "total_share",
        )
    ))
    if df_basic.empty:
    ...
    # 2. 行业映射一次（申万 L1）
    try:
        _ind_rows = list(AIndustryClass.objects.filter(
            src="SW2021", level="L1", out_date__isnull=True,
        ).values("ts_code", "index_name"))
        df_industry = pd.DataFrame(_ind_rows).rename(columns={"index_name": "industry_name"}) if _ind_rows else pd.DataFrame(columns=["ts_code", "industry_name"])
    except Exception as e:
